...
### Read resolved types and constant values

A resolved type is a structured node rather than a string, and a constant value
is data rather than a rendering. Dispatch over both with `isinstance`:
...
Because a resolved type is analysis output rather than syntax, it carries no
`span` and does not appear in `children()`.

### Compare and hash trees

Nodes, resolved types, and constant values compare and hash structurally. Source
position does not participate, so the same construct at a different offset is
still equal, and nodes work as `set` members and `dict` keys:
...
`parser.dumps` (also exported as `qdk.openqasm.dumps`) emits the
