...
class TrotterStep:
    ...
    evolution time ``time``. When ``controlled_on`` is set, the Cirq operations
    generated by :meth:`cirq` are controlled by that line-qubit index. This is
    useful when constructing controlled time evolution for quantum phase
    estimation.
    ...
    def __init__(
        ...
        terms: list[int] = [],
        time_step: float = 0.0,
        controlled_on: Optional[int] = None,
    ):
        ...
        Args:
            ...
            controlled_on: Optional line-qubit index that controls every Cirq
                operation generated for the step. The control qubit must be
                distinct from the model qubits. If ``None``, the generated
                operations are uncontrolled.
        ...
        self._controlled_on = controlled_on
    ...
    def cirq(self, model: Model) -> cirq.Circuit:
        ...
        Returns:
            ...
            in the same order as ``self.step()``. If ``controlled_on`` was
            provided at construction, each phasor is controlled by the
            corresponding ``cirq.LineQubit``.
        """
        ...
        for time, term_index in self.step():
            for color in model.colors(term_index):
                for op in model.ops(term_index, color):
                    ...
                    if self._controlled_on is not None:
                        control_qubit = cirq.LineQubit(self._controlled_on)
                        oper = cirq.PauliStringPhasor(
                            pauli, exponent_neg=time / math.pi
...
def suzuki_recursion(trotter: TrotterStep) -> TrotterStep:
    ...
    Returns:
        A new TrotterStep of order k+2 constructed via Suzuki recursion. The
        input step's control-qubit setting is preserved.

    References:
        M. Suzuki, Phys. Lett. A 146, 319 (1990).
    """

    suzuki = TrotterStep(controlled_on=trotter._controlled_on)
...
def yoshida_recursion(trotter: TrotterStep) -> TrotterStep:
    ...
    Returns:
        A new TrotterStep of order k+2 constructed via Yoshida recursion. The
        input step's control-qubit setting is preserved.
    ...
    yoshida = TrotterStep(controlled_on=trotter._controlled_on)
...
def strang_splitting(
    terms: list[int], time: float, controlled_on: Optional[int] = None
) -> TrotterStep:
    """
    ...
    Args:
        ...
        controlled_on: Optional line-qubit index that controls every Cirq
            operation generated for the step. If ``None``, the operations are
            uncontrolled.
    ...
    """
    strang = TrotterStep(controlled_on=controlled_on)
...
def fourth_order_trotter_suzuki(
    terms: list[int], time: float, controlled_on: Optional[int] = None
) -> TrotterStep:
    """
    ...
    Args:
        terms: Ordered term indices for the decomposition. Must be non-empty.
        time: Total evolution time assigned to the fourth-order step.
        controlled_on: Optional line-qubit index that controls every Cirq
            operation generated for the step. If ``None``, the operations are
            uncontrolled.

    Returns:
        A fourth-order ``TrotterStep`` that retains the requested control qubit.
    """
    return suzuki_recursion(strang_splitting(terms, time, controlled_on=controlled_on))
...
class TrotterExpansion:
    ...
    where ``dt = time / num_steps`` and then repeats it ``num_steps`` times. If
    ``controlled_on`` is set, every operation in the repeated Cirq circuit is
    controlled by that line-qubit index, enabling use in quantum phase
    estimation.
    ...
    def __init__(
        self,
        trotter_method: Callable[..., TrotterStep],
        model: Model,
        time: float,
        num_steps: int,
        controlled_on: Optional[int] = None,
        ...
        Args:
            trotter_method: Callable mapping ``(terms, dt)`` to a
                ``TrotterStep``. For controlled evolution, the callable must
                also accept the ``controlled_on`` keyword argument.
            model: Model that defines term groups and per-term operators.
            time: Total evolution time.
            num_steps: Number of repeated Trotter steps.
            controlled_on: Optional line-qubit index that controls every Cirq
                operation generated for the expansion. The control qubit must
                be distinct from the model qubits. If ``None``, the generated
                operations are uncontrolled.
        """
        self._model = model
        self._num_steps = num_steps
        if controlled_on is None:
            self._trotter_step = trotter_method(model.terms, time / num_steps)
        else:
            self._trotter_step = trotter_method(
                model.terms, time / num_steps, controlled_on=controlled_on
            )
    ...
    def cirq(self) -> cirq.CircuitOperation:
        """Get a repeated, optionally controlled Cirq circuit operation."""
