...
- ``qiskit`` — installs ``qiskit`` and makes ``qsharp.interop.qiskit`` available as ``qdk.qiskit``.
- ``cirq`` — installs ``cirq-core`` + ``cirq-ionq`` and makes ``qsharp.interop.cirq`` available as ``qdk.cirq``.
