...
pub(crate) use nodes::{Annotation, ClassicalType, Expression, QASMNode, Statement};
pub(crate) use semantic::{
    SemExpr, SemHardwareQubit, SemProgram, SemStmt, SemSymbol, SemSymbolTable, SemType,
};
pub(crate) use source::{
    Position, PositionEncoding, SourceDocument, SourceFile, SourceMap, SourceRange,
};
...
impl ParseResult {
    ...
    fn __repr__(&self) -> String {
        format!(
            ...
            py_bool(self.has_errors),
            py_items(self.diagnostics.len())
        )
    }
}
...
fn parse(
    ...
    let result = qdk_openqasm::parse_and_resolve(source, path, Some(&mut resolver));
...
fn analyze(
    ...
    let mut resolver = resolver_for(py, includes);
    let result = qdk_openqasm::analyze_and_resolve(source, path, Some(&mut resolver));
    let diagnostics = result
        .all_errors()
        .iter()
        .map(|error| diagnostic_from(error, &result.source_snapshot))
    ...
    let has_errors = result.has_errors();
    let document = Py::new(py, SourceDocument::from_snapshot(&result.source_snapshot))?;
    // One context spans both projections so they share interned type objects.
    let ctx = semantic::BuildContext::new(&result.symbols);
    let program = build_semantic_program(py, &result.program, &ctx, document.clone_ref(py))?;
    let symbols = build_symbol_table(py, &ctx)?;
    Ok(AnalysisResult {
        program,
        document,
        symbols,
...
fn qasm_dumps(py: Python<'_>, program: PyRef<'_, Program>) -> PyResult<String> {
    ...
    let result = qdk_openqasm::parse_and_resolve(
        source,
        path,
    ...
    let native_program = result.program();
...
pub(crate) fn verify_interop_struct_traits() {
    ...
    is_send_sync::<crate::openqasm::QASMNode>();
    is_send_sync::<crate::openqasm::Expression>();
    is_send_sync::<crate::openqasm::Statement>();
    is_send_sync::<crate::openqasm::ClassicalType>();
    is_send_sync::<crate::openqasm::Program>();
    is_send_sync::<crate::openqasm::QuantumGateModifier>();
    is_send_sync::<crate::openqasm::Span>();
    is_send_sync::<crate::openqasm::Diagnostic>();
    is_send_sync::<crate::openqasm::Label>();
    is_send_sync::<crate::openqasm::Severity>();
    is_send_sync::<crate::openqasm::PositionEncoding>();
    is_send_sync::<crate::openqasm::Position>();
    is_send_sync::<crate::openqasm::SourceRange>();
    is_send_sync::<crate::openqasm::SourceFile>();
    is_send_sync::<crate::openqasm::SourceMap>();
    is_send_sync::<crate::openqasm::SourceDocument>();
    ...
    is_send_sync::<crate::openqasm::SemProgram>();
    is_send_sync::<crate::openqasm::SemExpr>();
    is_send_sync::<crate::openqasm::SemStmt>();
    is_send_sync::<crate::openqasm::SemHardwareQubit>();
    is_send_sync::<crate::openqasm::SemType>();
    is_send_sync::<crate::openqasm::SemSymbol>();
    is_send_sync::<crate::openqasm::SemSymbolTable>();
}
