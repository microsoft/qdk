...
///   errors are positively correlated; it is slightly more work to compute; and
///   for small ``p_i`` it barely differs from the union-bound sum, so the two
///   only diverge noticeably in the high-error regime.
