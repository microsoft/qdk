...
use crate::openqasm::repr::{py_opt_str, py_str};
...
impl Severity {
    /// The Python spelling of this severity, reusable by other `__repr__` bodies.
...
impl Label {
    fn __repr__(&self) -> String {
        format!(
            "Label(span={}, message={})",
            self.span.py_repr(),
            py_opt_str(self.message.as_deref())
        )
    }
}
...
impl Diagnostic {
    ...
    /// same style used elsewhere in the QDK to display diagnostics. It is
    /// produced on demand, so reading `message` or `labels` costs nothing extra.
    fn __str__(&self) -> String {
        render_diagnostic(self)
    }
