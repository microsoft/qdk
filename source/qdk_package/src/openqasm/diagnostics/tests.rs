...
fn projects_cross_source_primary_and_related_labels() {
    ...
    let result = qdk_openqasm::parse_and_resolve(source, "main.qasm", Some(&mut resolver));
    ...
    assert!(projected.__str__().contains("main.qasm"));
    assert!(projected.__str__().contains("child.inc"));
}
