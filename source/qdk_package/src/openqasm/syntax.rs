...
//! projects the raw parse tree produced by [`qdk_openqasm::parse_and_resolve`].
//! The syntactic tree carries no resolved type, const-value, or symbol
//! information, so its nodes extend [`Expression`] / [`Statement`] directly (a
//! three-level `QASMNode -> Expression|Statement -> concrete` chain) rather than
//! through the semantic `SemExpr` / `SemStmt` bases.
//!
