...
def test_symbol_table_lookup_by_id() -> None:
    ...
    operand = gate.qubits[0]
    assert isinstance(operand, Identifier)
    # A node exposes its resolved symbol, and that symbol's own id is what
    # addresses the table.
    symbol = result.symbols.get(operand.symbol.id)
    assert isinstance(symbol, Symbol)
    assert symbol.name == "q"
...
def test_gate_definition_parameters_carry_spans_and_resolved_types() -> None:
    source = "OPENQASM 3.0;\ngate mygate(theta) q { rz(theta) q; }\n"
    result = semantic.analyze(source)
    definition = result.program.statements[-1]

    assert [parameter.name for parameter in definition.params] == ["theta"]
    assert [parameter.name for parameter in definition.qubits] == ["q"]

    # Each formal parameter is locatable in the source through its own span.
    for parameter in list(definition.params) + list(definition.qubits):
        assert isinstance(parameter.span, Span)
        assert source[parameter.span.lo : parameter.span.hi] == parameter.name
        assert result.document.source_map.range_from_span(parameter.span).source_id == 0

    # And carries the resolved type analysis gave it, not just a name.
    assert definition.params[0].type.name == "angle"
    assert definition.qubits[0].type.name == "qubit"
    assert definition.params[0].symbol.name == "theta"
