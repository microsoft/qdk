...
def test_binary_expression_operands() -> None:
    ...
    assert init.op == parser.BinaryOperator.ADD
    assert init.op.value == "+"
    assert init.lhs.value == 1
    assert init.rhs.value == 2


def test_io_declaration_keyword_is_enum_typed() -> None:
    result = parser.parse("OPENQASM 3.0; input int[8] a; output bit b;")
    keywords = [statement.io_keyword for statement in result.program.statements]
    assert keywords == [parser.IOKeyword.INPUT, parser.IOKeyword.OUTPUT]
...
def test_parse_program_strict_parses_once_and_preserves_error_payload(
    ...
    monkeypatch.setattr(parser, "_parse", spy)
...
def test_parse_program_success_parses_once_and_returns_same_program(
...
def test_subroutine_parameters_preserve_name_and_type_grouping() -> None:
    ...
    assert [
        parameter.type.size.value if parameter.type.size is not None else None
...
def test_array_reference_parameters_preserve_type_grouping() -> None:
    ...
    static, dynamic = (parameter.type for parameter in syntax_params)
    assert type(static).__name__ == "StaticArrayReferenceType"
    assert static.mutability == parser.AccessControl.READONLY
    assert type(static.base_type).__name__ == "IntType"
    # The element width and the dimension are distinguishable, unlike a flat list.
    assert static.base_type.size.value == 8
    assert [dimension.value for dimension in static.dimensions] == [2]

    assert type(dynamic).__name__ == "DynArrayReferenceType"
    assert dynamic.mutability == parser.AccessControl.MUTABLE
    assert type(dynamic.base_type).__name__ == "FloatType"
    assert dynamic.base_type.size.value == 32
    assert dynamic.num_dimensions.value == 2
    ...
    # The resolved type replaces the flat expression list the parameters used to
    # carry, so the element type and the dimensions are separately addressable.
    static_ty, dynamic_ty = (parameter.type for parameter in semantic_params)
    assert type(static_ty).__name__ == "StaticArrayReferenceType"
    assert type(static_ty.base_type).__name__ == "IntType"
    assert static_ty.base_type.size == 8
    assert static_ty.dimensions == [2]
    assert type(dynamic_ty).__name__ == "DynArrayReferenceType"
    assert type(dynamic_ty.base_type).__name__ == "FloatType"
    assert dynamic_ty.base_type.size == 32
    assert dynamic_ty.num_dimensions == 2
    assert all(parameter.symbol.name == parameter.name for parameter in semantic_params)
...
def test_syntax_type_fields_use_canonical_schema_and_preserve_expressions() -> None:
    ...
    # An array's element width and its dimensions stay distinguishable.
    assert type(array_decl.type).__name__ == "ArrayType"
    assert type(array_decl.type.base_type).__name__ == "IntType"
    assert array_decl.type.base_type.size.value == 8
    assert [dimension.value for dimension in array_decl.type.dimensions] == [2, 3]

    assert type(const_decl.type).__name__ == "UintType"
    assert const_decl.type.size.value == 16

    assert type(subroutine.return_type).__name__ == "ComplexType"
    assert type(subroutine.return_type.base_type).__name__ == "FloatType"
    assert subroutine.return_type.base_type.size.value == 64

    dyn_param, bit_param = external.param_types
    assert type(dyn_param).__name__ == "DynArrayReferenceType"
    assert dyn_param.mutability == parser.AccessControl.MUTABLE
    assert dyn_param.base_type.size.value == 20
    assert dyn_param.num_dimensions.value == 2
    assert type(bit_param).__name__ == "BitType"
    assert bit_param.size.value == 7
    assert type(external.return_type).__name__ == "IntType"
    assert external.return_type.size.value == 9

    assert type(for_loop.type).__name__ == "UintType"
    assert for_loop.type.size.value == 5
    assert type(input_decl.type).__name__ == "BitType"
    assert input_decl.type.size.value == 6
    assert type(cast_decl.type).__name__ == "IntType"
    assert cast_decl.type.size.value == 10
    assert type(cast.type).__name__ == "IntType"
    assert cast.type.size.value == 11

    # Every type node is reachable through the shared abstract base.
    assert isinstance(array_decl.type, parser.ClassicalType)
    assert isinstance(cast.type, parser.ClassicalType)
