...
@pytest.fixture
def c4_source_isa(c4_qodec: qc.Qodec) -> qc.InstructionSet:
...
@pytest.fixture
def c4_sampler(c4_qodec: qc.Qodec) -> StimSampler:
    return StimSampler(c4_qodec)
...
def test_noiseless_idle_has_no_detections(c4_sampler: "StimSampler", c4_source_isa: qc.InstructionSet) -> None:
...
def test_noisy_idle_has_some_detections(c4_qodec: qc.Qodec, c4_source_isa: qc.InstructionSet) -> None:
    sampler = StimSampler(
        c4_qodec, noise={"p_data": 0.1, "p_meas": 0.1}
    )
    program = _program(c4_source_isa, "prepare_zz", "idle")
...
def test_detector_error_model_uses_target_noise(
    c4_qodec: qc.Qodec,
    c4_source_isa: qc.InstructionSet,
) -> None:
    program = _program(c4_source_isa, "prepare_zz", "idle")
...
def test_prepare_measure_noiseless(c4_sampler: "StimSampler", c4_source_isa: qc.InstructionSet) -> None:
...
def test_prepare_measure_noisy(c4_qodec: qc.Qodec, c4_source_isa: qc.InstructionSet) -> None:
    sampler = StimSampler(
        c4_qodec, noise={"p_data": 0.05, "p_meas": 0.05}
    )
    program = _program(c4_source_isa, "prepare_zz", "measure_zz")
...
def test_sample_result_attributes(c4_sampler: "StimSampler", c4_source_isa: qc.InstructionSet) -> None:
    program = _program(c4_source_isa, "prepare_zz", "measure_zz")
