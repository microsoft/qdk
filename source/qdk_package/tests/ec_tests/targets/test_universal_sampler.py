...
``c4`` qodec: paulimer outcome-specific physical simulation plus the trivial
...
def _call(mnemonic: str, **operands: str) -> qc.instructions.InstructionCall:
...
def test_satisfies_sampler_protocol(c4_qodec: qc.Qodec) -> None:
    assert isinstance(UniversalSampler(c4_qodec), Sampler)


def test_only_construction_parameter_is_the_qodec(c4_qodec: qc.Qodec) -> None:
    sampler = UniversalSampler(c4_qodec)
    assert sampler.qodec is c4_qodec


def test_z_memory_is_noiseless(c4_qodec: qc.Qodec) -> None:
    program = Program(
        [
    ...
    batch = UniversalSampler(c4_qodec).execute(program, shots=200)
...
def test_x_memory_is_noiseless(c4_qodec: qc.Qodec) -> None:
    program = Program(
        [
    ...
    bits = np.asarray(UniversalSampler(c4_qodec).execute(program, shots=100), bool)
    assert not bits.any()


def test_transversal_cx_correlates_logical_outcomes(c4_qodec: qc.Qodec) -> None:
    ...
    bits = np.asarray(UniversalSampler(c4_qodec).execute(program, shots=200), bool)
    assert (bits[:, :2] == bits[:, 2:4]).all()


def test_shots_independent_trajectories(c4_qodec: qc.Qodec) -> None:
    program = Program([_call("prepare_zz", block="data")], c4_qodec.layers[0].isa)
    batch = UniversalSampler(c4_qodec).execute(program, shots=8)
    # prepare_zz declares no observe outcomes, so each shot is an empty row.
    assert len(batch) == 8
    assert all(len(row) == 0 for row in batch)


def test_assume_satisfied_passes(c4_qodec: qc.Qodec) -> None:
    ...
    batch = UniversalSampler(c4_qodec).execute(program, shots=100)
    assert len(batch) == 100


def test_assume_violation_raises(c4_qodec: qc.Qodec) -> None:
    ...
    with pytest.raises(AssumeViolation):
        UniversalSampler(c4_qodec).execute(program, shots=100)


def test_no_spurious_warnings_for_supported_program(c4_qodec: qc.Qodec) -> None:
    program = Program(
    ...
    with warnings.catch_warnings():
        ...
        UniversalSampler(c4_qodec).execute(program, shots=10)
