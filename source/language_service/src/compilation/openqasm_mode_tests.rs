...
/// Guards the stage-1 aggregation contract: parser errors must be included
/// exactly once alongside semantic errors.
#[test]
fn syntax_errors_are_not_duplicated() {
    ...
    assert_eq!(rendered.len(), res.all_errors().len());
    assert_eq!(rendered.len(), res.parse_errors().len());
}
