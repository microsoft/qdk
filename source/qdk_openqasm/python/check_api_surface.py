...
"""Lint: detect private-symbol leakage in the qdk_openqasm_parser public API surface.
...
ROOT_PACKAGE = "qdk_openqasm_parser"
...
# __module__ paths.
OWNED_PREFIXES: tuple[str, ...] = ("qdk_openqasm_parser.",)
...
def _module_has_private_segment(mod_name: str) -> bool:
    ...
    >>> _module_has_private_segment("qdk_openqasm_parser._native")
    True
    >>> _module_has_private_segment("qdk_openqasm_parser.visitors")
    False
    """
...
def _build_public_types(
    ...
    Returns:
        ...
        - public_type_names is a set of unqualified names (e.g. "Stmt")
...
def _check_class(
    ...
    reference directly, so it does not constitute actionable leakage.
    """
    # Check public methods — only those defined in owned modules.
...
def _import_root_package() -> types.ModuleType:
    """Import the *installed* package, not the in-tree source.

    This script lives inside the package source directory, alongside the
    ``qdk_openqasm_parser/`` source tree.  When run as
    ``python check_api_surface.py``, Python puts that directory at
    ``sys.path[0]``, so a bare ``import qdk_openqasm_parser`` resolves to the
    *source* package -- which has no compiled native extension
    (``qdk_openqasm_parser._native``) and therefore fails to import.
...
def _iter_modules() -> list[tuple[str, types.ModuleType]]:
    """Import and yield all public submodules."""
...
def scan() -> list[Violation]:
    """Scan the package and return all violations."""
    violations: list[Violation] = []
