...
use crate::compat::{IndexMap, Span};
use core::f64;
...
impl SymbolTable {
    ...
    /// Returns an iterator over all symbols in the table, yielding each
    /// symbol's [`SymbolId`] paired with a reference to the symbol.
    pub fn iter(&self) -> impl Iterator<Item = (SymbolId, &Rc<Symbol>)> {
        self.symbols.iter()
    }

    /// Gets the symbol with the given ID without panicking, returning [`None`]
    /// if no symbol with that ID exists.
    #[must_use]
    pub fn get(&self, id: SymbolId) -> Option<&Rc<Symbol>> {
        self.symbols.get(id)
    }
        ...
        S: AsRef<str>,
    {
        ...
        if let Some(scope) = last_false
            && let Some((id, symbol)) = scope.get_symbol_by_name(name.as_ref())
        {
    ...
    #[must_use]
    pub fn get_input(&self) -> Option<Vec<Rc<Symbol>>> {
    ...
    #[must_use]
    pub fn get_output(&self) -> Option<Vec<Rc<Symbol>>> {
...
#[cfg(test)]
mod tests {
    use super::{IOKind, Span, Symbol, SymbolId, SymbolTable};
    use crate::semantic::types::Type;

    #[test]
    fn iter_yields_all_symbols_and_get_returns_none_for_missing_id() {
        ...
        // `iter()` should surface every inserted symbol, including the one we
        // just added and the built-in symbols populated by `default()`.
        let collected: Vec<_> = table.iter().collect();
        assert!(collected.iter().any(|(sym_id, _)| *sym_id == id));
        assert!(
            collected
                .iter()
                .any(|(_, symbol)| symbol.name == "my_symbol")
        );

        // `get()` returns the symbol for a valid id.
        let fetched = table.get(id).expect("symbol should be present");
        assert_eq!(fetched.name, "my_symbol");

        // `get()` returns `None` (instead of panicking) for an unknown id.
        let missing = SymbolId(
            u32::try_from(collected.len()).expect("symbol count should fit in u32") + 1_000,
        );
