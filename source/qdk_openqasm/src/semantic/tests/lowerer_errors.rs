...
#[test]
fn unhandled_literal_cast_pair_is_diagnosed_rather_than_aborting() {
    // `can_cast_literal` admits uint from complex, but no coercion arm handles
    // it, and the non-literal path already rejects the same cast.
    check_qasm(
        "OPENQASM 3.0;\nuint v = 1im;\n",
        &expect![[r#"
            Qdk.Qasm.Lowerer.CannotCastLiteral

              x cannot cast literal expression of type const complex[float] to type uint
               ,-[Test.qasm:2:10]
             1 | OPENQASM 3.0;
             2 | uint v = 1im;
               :          ^^^
...
#[test]
fn angle_accepts_a_bit_literal_of_matching_width() {
    // The same cast through a variable was already accepted, so the literal
    // form must be too rather than reaching the unhandled-pair path.
    crate::semantic::tests::check_classical_decl(
        "angle[4] v = \"1010\";",
        &expect![[r#"
            ClassicalDeclarationStmt [0-20]:
                ...
                ty_exprs:
                    Expr [6-7]:
                        ...
                        const_value: Int(4)
                        kind: Lit: Int(4)
                init_expr: Expr [13-19]:
                    ty: const angle[4]
                    kind: Lit: Angle(3.9269908169872414)
            [8] Symbol [9-10]:
                name: v
                type: angle[4]
                ty_span: [0-8]
