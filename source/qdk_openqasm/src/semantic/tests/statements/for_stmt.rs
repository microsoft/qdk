...
use crate::{
    ...
    semantic::{
        ast::{EnumerableSet, StmtKind},
        parse_source,
        tests::check_stmt_kinds,
        types::Type,
    },
    tests::check_qasm,
};
...
#[test]
fn enumerable_ranges_and_sets_carry_resolved_types() {
    let source = "
        for int i in [0:3] {}
        for int i in {1, 2} {}
    ";
    ...
    let StmtKind::For(range_loop) = result.program.statements[0].kind.as_ref() else {
        panic!("expected range loop");
    };
    let EnumerableSet::Range(range) = range_loop.set_declaration.as_ref() else {
        panic!("expected range iterable");
    };
    assert_eq!(range.ty, Type::Range);

    let StmtKind::For(set_loop) = result.program.statements[1].kind.as_ref() else {
        panic!("expected set loop");
    };
    let EnumerableSet::Set(set) = set_loop.set_declaration.as_ref() else {
        panic!("expected set iterable");
    };
    assert_eq!(set.ty, Type::Set);
}
