...
impl Lowerer {
        ...
        for stmt in &program.statements {
            match &*stmt.kind {
                syntax::StmtKind::Include(include) => {
                    ...
                    let included_source = includes.next().expect("missing include");
                    self.lower_source(included_source, false);
                }
    ...
    fn get_alias_ty_from_exprs(&mut self, rhs: &Vec<Expr>) -> Type {
        ...
        for expr in rhs {
            match expr.ty {
                ...
                _ => {
                    // The loop above rejected every type outside this pair and
                    // returned `Type::Err`, so no source reaches this arm.
                    unreachable!("alias operands are qubit arrays or bit arrays");
                }
            }
        }
    ...
    fn get_concat_type_from_exprs(&mut self, rhs: &Vec<Expr>) -> Type {
        ...
        for expr in rhs {
            match &expr.ty {
                ...
                Type::RankedArrayRef(_) => {
                    // `all_same` forces every operand to match `first_ty`, and a
                    // `RankedArrayRef` first operand returned above.
                    unreachable!("`Type::RankedArrayRef` was handled above");
                }
                _ => {
                    // The loop above rejected every non-array type and returned
                    // `Type::Err`, so no source reaches this arm.
                    unreachable!("concatenation operands are arrays");
                }
            }
        }
        ...
        let base_ty = if let Type::Array(array_ty) = &first_ty {
            ...
            // Validation admitted only the three array types, and the
            // `RankedArrayRef` case returned above.
            unreachable!("`first_ty` is `Type::Array` or `Type::StaticArrayRef`")
        };
    ...
    fn lower_include(&mut self, stmt: &syntax::IncludeStmt) -> semantic::StmtKind {
        ...
        // A global-scope include should already have been inlined by `lower_source`. Reaching
        // here means that invariant broke, so report it rather than unwinding through the
        // library's Python and editor callers.
        let name = stmt.filename.to_string();
        let kind = SemanticErrorKind::IncludeNotResolved(name, stmt.span);
        self.push_semantic_error(kind);
        semantic::StmtKind::Err
    }
    ...
    pub(crate) fn try_coerce_literal_expr_to_type(
        ...
        let result = match (&lhs_ty, &rhs_ty) {
            ...
            (Type::Angle(Some(angle_width), _), Type::BitArray(array_width, _))
                if angle_width == array_width =>
            {
                // The non-literal path accepts this cast, so the literal form must too.
                // The spec makes it an exact per-bit copy, little-endian.
                if let semantic::LiteralKind::Bitstring(value, _) = kind
                    && let Ok(value) = u64::try_from(value)
                {
                    return Ok(semantic::Expr::new(
                        span,
                        semantic::ExprKind::Lit(semantic::LiteralKind::Angle(Angle::new(
                            value,
                            *angle_width,
                        ))),
                        lhs_ty.as_const(),
            ...
            (Type::Bit(..), Type::Int(..) | Type::UInt(..)) => {
                ...
                if let semantic::LiteralKind::Int(value) = kind {
                    ...
                    unreachable!("`can_cast_literal_with_value_knowledge` admitted the value");
                } else {
                    // A `Bit` target with an `Int`, `Bool`, or `Angle` literal
                    // returned at the top of this function, and the assertion
                    // above rejects every other literal kind for this pair.
                    unreachable!("a `Bit` target coerces its literal kinds above");
                }
            }
            (Type::Int(width, _), Type::Int(_, _) | Type::UInt(_, _)) => {
                ...
                match kind {
                    ...
                    // An `Int` or `UInt` typed literal is an `Int` or `BigInt`.
                    _ => unreachable!("an integer literal is `Int` or `BigInt`"),
                }
            }
            _ => None,
        };
        // `can_cast_literal` admits some pairs that no arm above coerces, so an
        // unhandled pair is a rejected cast rather than a reason to abort.
        result.ok_or_else(|| Self::invalid_literal_cast_error(ty, &rhs.ty, rhs.span))
    }
    ...
    fn cast_array_expr_to_type(ty: &Type, expr: &semantic::Expr) -> Option<semantic::Expr> {
        ...
        let Type::Array(array_ty) = &expr.ty else {
            // Restates the assertion above for the binding.
        ...
        let unwrap_range_component = |expr_opt: Option<&Expr>, default: i64| -> (i64, Span) {
            if let Some(expr) = expr_opt {
                let Some(semantic::LiteralKind::Int(val)) = expr.const_value else {
                    // `lower_const_range` const-evaluates every component and
                    // fails the whole index otherwise, so an index that reaches
                    // this point has integer components.
