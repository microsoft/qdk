...
#[test]
fn standalone_analysis_reports_unresolved_include() {
    let result = crate::analyze("OPENQASM 3.0;\ninclude \"gates.inc\";\nqubit q;");
    ...
    assert!(
        result
            .all_errors()
            .iter()
            .any(|error| format!("{error:?}").contains("Could not resolve include file")),
        "expected an unresolved include diagnostic, got {:?}",
        result.all_errors()
    );
}

#[test]
fn standalone_analysis_resolves_builtin_includes_without_a_resolver() {
    let result = crate::analyze("OPENQASM 3.0; include \"stdgates.inc\"; qubit q; h q;");
