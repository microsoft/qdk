...
# TODO: test compile,circuit,logcial_counts,set_quantum_seed,set_classical-seed,
...
def test_cross_session_callable_passing_raises() -> None:
    ...
    ctx_a = qsharp.Session()
    ctx_b = qsharp.Session()
    ...
    with pytest.raises(Exception, match="different Session"):
        ctx_b.logical_counts(foo)

# TODO: fix this test.
def test_cross_session_struct_passing_raises() -> None:
    """Passing a struct value from one context to another's function."""
    ctx_a = qsharp.Session()
    ctx_b = qsharp.Session()
    ...
    # Create a Point struct instance in ctx_a
    point_from_ctx_a = ctx_a.code.Point(3, 4)
    assert ctx_a.code.ProcessPoint(point_from_ctx_a) == 7

    with pytest.raises(QSharpError, match="different Session"):
        ctx_b.code.ProcessPoint(point_from_ctx_a)
