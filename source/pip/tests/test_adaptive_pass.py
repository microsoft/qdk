...
from qsharp._adaptive_pass import AdaptiveProfilePass
from qsharp._adaptive_opcodes import (
    FLAG_SRC0_IMM,
    OP_ADD,
    OP_AND,
    OP_BRANCH,
...
def _run_pass(ir: str, name: str = "test.ll") -> dict:
...
def test_linear_quantum_ops():
    ...
    r = _run_pass(LINEAR_QIR)
    op_ids = [q[0] for q in r["quantum_ops"]]
...
def test_linear_instruction_opcodes():
    ...
    r = _run_pass(LINEAR_QIR)
    primaries = [_primary(inst[0]) for inst in r["instructions"]]
    ...
    assert _primary(r["instructions"][-1][0]) == OP_RET
...
def test_linear_static_qubit_sentinel():
    ...
    for inst in r["instructions"]:
        if _primary(inst[0]) == OP_QUANTUM_GATE:
            # aux1 and aux2 should be DYN_QUBIT_SENTINEL for static qubits
            assert (
                inst[5] == DYN_QUBIT_SENTINEL
            ), f"Expected DYN_QUBIT_SENTINEL in aux1, got {inst[5]}"
            assert (
                inst[6] == DYN_QUBIT_SENTINEL
            ), f"Expected DYN_QUBIT_SENTINEL in aux2, got {inst[6]}"
            return
...
def test_measure_correct_branch():
    ...
    _, offset, count, _ = entry_block
    last_instr = r["instructions"][offset + count - 1]
    assert _primary(last_instr[0]) == OP_BRANCH
...
def test_measure_correct_read_result_bool_type():
    ...
    r = _run_pass(MEASURE_CORRECT_QIR)
    for inst in r["instructions"]:
        if _primary(inst[0]) == OP_READ_RESULT:
            dst_reg = inst[1]
...
def test_measure_correct_unconditional_jump():
    ...
    _, offset, count, _ = then_block
    last_instr = r["instructions"][offset + count - 1]
    assert (
        _primary(last_instr[0]) == OP_JUMP
    ), f"Expected OP_JUMP at end of 'then' block, got {_primary(last_instr[0]):#x}"
...
def test_loop_phi_entries():
    ...
    block_ids = {pe[0] for pe in r["phi_entries"]}
...
def test_loop_phi_forward_ref_reuse():
    ...
    back_edge_entry = next(pe for pe in r["phi_entries"] if pe[0] == 1)
    next_reg = back_edge_entry[1]

    # Find the ADD instruction (for %next = add %i, 1)
    for inst in r["instructions"]:
        if _primary(inst[0]) == OP_ADD:
            add_dst = inst[1]
            assert (
                add_dst == next_reg
...
def test_loop_icmp_and_branch():
    ...
    _, offset, count, _ = loop_block
    instrs = r["instructions"][offset : offset + count]
    primaries = [_primary(inst[0]) for inst in instrs]
...
def test_loop_icmp_ult_sub_opcode():
    ...
    r = _run_pass(LOOP_PHI_QIR)
    for inst in r["instructions"]:
        if _primary(inst[0]) == OP_ICMP:
            assert (
                _sub(inst[0]) == ICMP_ULT
            ), f"Expected ICMP_ULT={ICMP_ULT} sub-opcode, got {_sub(inst[0])}"
            return
    pytest.fail("No OP_ICMP found")


def test_loop_const_materialization():
    """Integer constants in the loop emit OP_CONST with FLAG_SRC0_IMM."""
    r = _run_pass(LOOP_PHI_QIR)
    const_instrs = [inst for inst in r["instructions"] if _primary(inst[0]) == OP_CONST]
    assert len(const_instrs) > 0, "Expected at least one OP_CONST for loop constants"
    for inst in const_instrs:
        assert (
            _flags(inst[0]) & FLAG_SRC0_IMM
        ), f"OP_CONST should have FLAG_SRC0_IMM set, got flags {_flags(inst[0]):#x}"
...
def test_loop_entry_unconditional_jump():
    ...
    assert (
        _primary(last_instr[0]) == OP_JUMP
    ), f"Expected OP_JUMP at end of entry block, got {_primary(last_instr[0]):#x}"
...
def test_classical_boolean_read_results():
    ...
    r = _run_pass(CLASSICAL_BOOLEAN_QIR)
    read_count = sum(
        1 for inst in r["instructions"] if _primary(inst[0]) == OP_READ_RESULT
    )
    assert read_count == 2
...
def test_select_const_operands():
    """Select true/false values (i32 42 and 7) emit OP_CONST with FLAG_SRC0_IMM."""
    r = _run_pass(SELECT_QIR)
    const_instrs = [inst for inst in r["instructions"] if _primary(inst[0]) == OP_CONST]
    # The select has two i32 constants: 42 and 7
    const_values = {inst[2] for inst in const_instrs}  # src0 field holds the immediate
    assert 42 in const_values, f"Missing constant 42 in {const_values}"
    assert 7 in const_values, f"Missing constant 7 in {const_values}"
...
def test_reset_instruction():
    ...
    primaries = [_primary(inst[0]) for inst in r["instructions"]]
    assert OP_RESET in primaries, "Missing OP_RESET"


def test_reset_static_qubit_sentinel():
    """Static-qubit reset uses DYN_QUBIT_SENTINEL in aux1."""
    r = _run_pass(RESET_QIR)
    for inst in r["instructions"]:
        if _primary(inst[0]) == OP_RESET:
            assert (
                inst[5] == DYN_QUBIT_SENTINEL
            ), f"Expected DYN_QUBIT_SENTINEL in aux1, got {inst[5]}"
            return
    pytest.fail("No OP_RESET found")
...
def test_bell_loop_dynamic_qubit_no_sentinel():
    ...
    for inst in r["instructions"]:
        if _primary(inst[0]) == OP_QUANTUM_GATE:
            if inst[5] != DYN_QUBIT_SENTINEL:
                has_dynamic = True
                # The dynamic reg must be a valid register index
                assert (
                    0 <= inst[5] < r["num_registers"]
                ), f"Invalid dynamic qubit register: {inst[5]}"
...
def test_bell_loop_funcs_call_instruction():
    ...
    r = _run_pass(BELL_LOOP_FUNCS_QIR)
    call_instrs = [inst for inst in r["instructions"] if _primary(inst[0]) == OP_CALL]
    ...
    call = call_instrs[0]
    assert call[4] == 0, f"OP_CALL aux0 (func_id) should be 0, got {call[4]}"
    # aux1 = num_args (2 qubit pointers)
    assert call[5] == 2, f"OP_CALL aux1 (num_args) should be 2, got {call[5]}"
...
def test_bell_loop_funcs_call_args():
    ...
    call_instrs = [inst for inst in r["instructions"] if _primary(inst[0]) == OP_CALL]
    call = call_instrs[0]
    arg_offset = call[6]  # aux2 = arg_offset into call_args
    num_args = call[5]  # aux1 = num_args
...
def test_bell_loop_funcs_call_return():
    ...
    func_entry_block = r["functions"][0][0]
    # Find the block tuple for that block
    func_block = next(b for b in r["blocks"] if b[0] == func_entry_block)
    _, offset, count, _ = func_block
    last_instr = r["instructions"][offset + count - 1]
    assert (
        _primary(last_instr[0]) == OP_CALL_RETURN
    ), f"make_bell should end with OP_CALL_RETURN, got {_primary(last_instr[0]):#x}"
...
def test_bell_loop_funcs_param_registers():
    ...
    r = _run_pass(BELL_LOOP_FUNCS_QIR)
    func = r["functions"][0]
    _, num_params, param_base, _ = func
...
def test_bell_loop_phi_block_refs():
    ...
    r = _run_pass(BELL_LOOP_QIR)
    valid_block_ids = {b[0] for b in r["blocks"]}
    for block_id, val_reg in r["phi_entries"]:
...
def test_bell_loop_forward_ref_consistency():
    ...
    back_edge_regs = set()
    for block_id, val_reg in r["phi_entries"]:
        if block_id in (2, 4):  # back-edge blocks
            back_edge_regs.add(val_reg)
