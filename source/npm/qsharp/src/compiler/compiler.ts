...
export interface ICompiler {
  ...
  /**
   * Generate Q# source code that implements the given circuit.
   *
   * @param fileName Used to derive the operation name(s) in the generated Q# code.
   * @param circuits The circuit (or circuit group) to convert.
   */
  circuitsToQsharp(fileName: string, circuits: CircuitData): Promise<string>;
...
export class Compiler implements ICompiler {
  ...
  async circuitsToQsharp(
    fileName: string,
    circuits: CircuitData,
  ): Promise<string> {
    return callAndTransformExceptions(async () =>
      this.wasm.circuits_to_qsharp(fileName, JSON.stringify(circuits)),
    );
  }
