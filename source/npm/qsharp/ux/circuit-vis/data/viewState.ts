...
/**
 * Per-session view preferences that survive `Sqore.renderCircuit()` but are NOT persisted to the
 * saved circuit (`.qsc`) file.
 *
 ...
 *
 * `ViewState` holds state the user expects to stay stable while editing but that doesn't belong in
 * the file — chiefly per-group expand/collapse.
 *
 * # Override semantics
 *
 * Only explicit user choices are stored. Default expansion is computed per-render in
 * [sqore.ts](../sqore.ts); user overrides are applied after via [`applyTo`](#method-applyTo):
 *
...
export class ViewState {
  /**
   * Map from op location string to the user's explicit expansion choice. Absent = no choice (use
   * defaults).
   */
  readonly expanded = new Map<string, boolean>();

  /**
   * Record the user's choice to expand or collapse the op at `location`. Idempotent.
   *
   * Collapsing also clears explicit overrides on descendants of `location`, so re-expanding later
  ...
  /**
   * Drop the user's choice for the op at `location`, falling back to the default-expansion logic on
   * the next render. Idempotent; absence of an entry is fine.
   */
   ...
   *   - `remap.get(oldKey) === null` → drop the entry (op is no longer in the grid).
   *   - `remap.has(oldKey) === false` → leave unchanged (no info about this op; keep rather than
   *     guess).
   *
   * The View layer ([sqore.ts](../sqore.ts)) calls this each render to track ops whose locations
   * shifted due to upstream edits. The remap is computed by object identity against the previous
   * render's snapshot, so unmoved ops keep their key even when their string location would
   * otherwise drift. Idempotent against a fixed-point remap; on key collisions, later writes win.
   */
  rebase(remap: ReadonlyMap<string, string | null>): void {
    // Build the rebased map in a fresh container, then swap it in. Two passes (read-only iteration,
    // then atomic replacement) is what makes key chains (`a → b`, `b → c`) and key swaps (`a → b`,
    // `b → a`) correct — an in-place rekey would clobber an entry before its own rename had a
    // chance to run.
  ...
  /**
   * Apply user expansion overrides to a freshly-rendered component grid. Walks `grid` recursively
   * and, for any op whose location string has a `ViewState.expanded` entry, sets
   * `dataAttributes.expanded` to `"true"` / `"false"` accordingly.
   *
   * Call AFTER the per-render default-expansion passes (`expandOperationsToDepth`,
