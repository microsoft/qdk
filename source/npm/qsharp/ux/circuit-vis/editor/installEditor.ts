 ...
 * installs every editor concern around it in one call — editor-only
 * DOM (toolbox, state-viz panel, dropzones, ghost-qubit row),
 * pointer + keyboard interaction (see [events.ts](events.ts)), and
 * the host's edit notification.
 *
 * Sqore re-invokes this on every `renderCircuit`, so the helpers it
 * calls are idempotent on re-call (e.g. [mountEditorShell](shell.ts)
 * reuses pre-existing DOM).
 *
 ...
 * @param layoutMap Geometry from the layout pass. See
