...
**✅ Resolved.** Public `removeQubitWithDependents(model, qubitIdx)`
added; it owns the strip-then-`removeQubit` cascade. The primitive
is demoted to a fully module-private `_findAndRemoveOperations` (no
longer exported). `qubitController` now makes a single call and
dropped the `getOperationRegisters` / `Operation` imports. The
`removeQubit` doc no longer points callers at the primitive. The
two in-action-layer measurement callers keep using the primitive,
which is correct: each still follows it with its own trimming
`remove*`. Tests follow the public surface — the flat decrement /
column-prune behavior is covered through `removeQubitWithDependents`
in [qubitOps.test.mjs](../../test/circuit-editor/circuit-actions/qubitOps.test.mjs),
and the group-strip / target-narrowing cases in
[groupAddRemove.test.mjs](../../test/circuit-editor/circuit-actions/groupAddRemove.test.mjs)
drive through the public `removeOperation` (remove a leaf, refresh
ancestors); no test imports the primitive.

### `makeDropzoneBox` API is too heavy — ✅ shipped
...
Testability half (earlier): extracted a pure
...
with bare primitives. `composeLocation` already delegates to the
data-layer [`Location`](data/location.ts) type, so there was no
duplicated address format to retire.

**Signature half (this pass).** The earlier candidate — an
"identity object" `{ pathPrefix, colIndex, opIndex }` — was
discarded after re-reading the body: the geometry/identity seam
is _not_ clean. `colIndex` feeds both `columnGeometry` (geometry)
and the location string (identity); `wireIndex` feeds both
`wireData[wireIndex]` and the `-wire` attr; `interColumn` picks
the box shape _and_ sets the `-inter-column` attr. Bundling
`{pathPrefix, colIndex, opIndex}` as "identity" would have been a
leaky abstraction (`colIndex` is also a primary geometry input).

Instead the args were split along **what actually varies**, which
matches the real data flow at every call site:

- [`DropzoneContext`](editor/draggable.ts) `{ scope, wireData,
pathPrefix? }` — the scope being drawn into. Loop-invariant at
  all four call sites, so each builds it once and reuses it.
- [`DropzoneTarget`](editor/draggable.ts) `{ colIndex, opIndex,
wireIndex, interColumn }` — the single cell + shape, varies per
  dropzone.

New signature: `makeDropzoneBox(ctx, target)`. Call sites no
longer pass bare positional `true`/`false`/`0` runs — the booleans
and indices are named at the call. All four callers
([`_populateDropzonesForGrid`](editor/draggable.ts),
[`_appendTrailingColumnForScope`](editor/draggable.ts),
[dragController.ts](editor/controllers/dragController.ts) ×2) hoist
the context out of their loops. Layering preserved: the structs and
the DOM `setAttribute` stay in `editor/` (View); `data/` + `actions/`
