...
const getClassicalControlRegisters = (op: Operation): Register[] =>
  op.kind === "unitary"
    ? [
        ...(op.controls ?? []).filter(
          (control) => control.result !== undefined,
        ),
 ...
 *
 * Only controls count, never the derived `.targets` cache. Controls are represented either by a
 * classical register in `.controls` or by a compact `.classicalControls` entry. Flagging
 * `.targets` would falsely mark every enclosing group and cascade-delete unrelated siblings.
 *
  ...
  const walk = (g: ComponentGrid, prefix: string): void => {
    g.forEach((col, ci) => {
      col.components.forEach((op, oi) => {
        ...
        const controls = getClassicalControlRegisters(op);
        if (
          controls.some((reg) => producedKeys.has(`${reg.qubit}:${reg.result}`))
        ) {
