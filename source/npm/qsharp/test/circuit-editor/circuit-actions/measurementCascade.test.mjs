...
test("collectSubtreeConsumers: finds a compact classical consumer", () => {
...
test("removeOperationWithDependents: deletes compact classical consumers", () => {
  const compactX = _compactControlled("X", 1, 0, 0);
  const model = build(circuit(2, [[_mGate(0, 0)], [compactX]]));
  const consumers = collectSubtreeConsumers(model.componentGrid, "0,0");
  removeOperationWithDependents(
...
test("moveOperationWithDependents: compact consumer tracks the M's new wire", () => {
  const compactX = _compactControlled("X", 2, 0, 0);
  const model = build(circuit(3, [[_mGate(0, 0)], [compactX]]));
  assert.equal(collectSubtreeConsumers(model.componentGrid, "0,0").length, 1);
