...
beforeEach(() => {
  ...
  // JSDOM doesn't implement DOMPoint or DOMMatrix at all. The `pickSelectedWire` closest-wire path
  // uses both: new DOMPoint(clientX, clientY).matrixTransform(ctm.inverse()) We stub a minimal
  // *identity* DOMPoint so clientY flows through to SVG-space Y unchanged — the CTM stub in
  // `buildMultiWireFixture` is wired to match (it returns an identity matrix whose `.inverse()` is
  // also identity, and `matrixTransform` on our stub just returns the point itself). This is enough
  // surface area to exercise the controller's coord-projection wiring without dragging an
  // SVG-layout shim into the test deps. Cast the assignment because our stub doesn't implement the
  ...
  globalThis.DOMPoint = /** @type {any} */ (
    class {
      ...
      // Identity transform: the fixture's CTM stub is also identity, so this is correct for all our
      // tests. A real layout-aware run would apply the matrix; we don't need that fidelity.
      matrixTransform() {
 ...
 * Returns the container plus a map of the host elements for direct event dispatch.
 */
...
/**
 * Construct a SelectionController with the minimum viable `InteractionContext` and a stub
 * `CircuitEvents`. Pass `wireData` (matching a multi-wire fixture) to exercise the closest-wire
 * path.
 *
...
function makeController(
  ...
  // Stub CircuitEvents — only used as a closure capture by addContextMenuToHostElem (never invoked
  // in these tests).
...
/**
 * Build a multi-wire fixture: a group body that spans wires 0..2, with `data-wire-ys = [40, 100,
 * 160]` and a (stale) static `data-wire = 0`. The closest-wire path should override the static
 * attribute and pick the wire closest to the click's SVG-space Y.
 *
 * Stubs `circuitSvg.getScreenCTM()` to the identity transform so `clientY` flows through to
 * SVG-space Y unchanged. JSDOM doesn't implement layout, so a real CTM call would return null and
 * the controller would fall back to the static `data-wire` — masking the closest-wire behavior
...
function buildMultiWireFixture(wireYs = [40, 100, 160]) {
  ...
  // Identity CTM hand-stub. JSDOM doesn't ship a DOMMatrix at all, and our DOMPoint stub's
  // `matrixTransform()` already returns the point unchanged — so all this needs is something
  // non-null with an `.inverse()` method the controller can call before passing it to
  // `matrixTransform`. The returned value never has to be a real matrix.
  ...
  // Static attr is the topmost-wire fallback, which the closest-wire path must override.
...
/**
 * Dispatch a primary-button mousedown at a given client Y so the closest-wire resolution has a
...
test("multi-wire host: falls back to data-wire when getScreenCTM returns null", () => {
  // Detached SVG / pre-mount edge case. The controller must not throw; it should fall back to the
  // static `data-wire` attribute so the click still resolves *some* wire.
  const { container, groupBody } = buildMultiWireFixture();
...
test("multi-wire host: falls back to data-wire if closest wireY is not in wireData", () => {
  // wireYs claims [40, 100, 160] but the editor's wireData is [200, 300] — table mismatch.
  // pickClosestWireIndex returns -1, the controller falls back to the static data-wire.
  const { container, groupBody } = buildMultiWireFixture();
...
test("single-wire host: closest-wire path is skipped, data-wire still wins", () => {
  // Smoke-check: the multi-wire branch must not engage for single-wire spans (control dots, target
  // circles, etc.). Use a host with data-wire-ys=[100] and data-wire=1 — controller must resolve to
  // wire 1 regardless of clickY.
  const container = document.createElement("div");
  document.body.appendChild(container);
  const svg = document.createElementNS(SVG_NS, "svg");
  svg.setAttribute("class", "qviz");
