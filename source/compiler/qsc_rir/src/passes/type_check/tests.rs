...
use crate::rir::{
    BlockId, Callable, CallableId, CallableType, InstructionKind, Literal, Operand, Program, Ty,
    Variable, VariableId,
};
...
fn binop_instr_matching_types_passes_check() {
    ...
    check_instr_types(&Program::new(), &InstructionKind::Add(opr1, opr2, var));
}
...
fn unop_instr_matching_types_passes_check() {
    ...
    check_instr_types(&Program::new(), &InstructionKind::BitwiseNot(opr, var));
}
...
fn phi_instr_matching_types_passes_check() {
    ...
    check_instr_types(
        &Program::new(),
        &InstructionKind::Phi(vec![(opr, BlockId(0)), (opr, BlockId(1))], var),
    );
}
...
fn call_instr_no_return_type_no_output_var_passes_check() {
    ...
    check_instr_types(
        &program,
        &InstructionKind::Call(CallableId(0), vec![opr], None),
    );
}
