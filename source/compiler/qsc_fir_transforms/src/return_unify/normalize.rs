...
pub(super) use anf::find_unsupported_operand_lifts;
pub(super) use anf::run_to_fixpoint as run_anf_to_fixpoint;
...
/// compound-position `Return` nodes (`count_compound_position_returns`) must
/// strictly decrease on each `changed = true` iteration. A hard cap guards
/// against unbounded looping.
///
