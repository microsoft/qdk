        ...
        &expect![[r#"
            callable Main: input_ty=Unit, output_ty=Result
                body: block_ty=Result
                    [0] Local(Mutable, _.has_returned: Bool): Lit(Bool(false))
                    [1] Local(Mutable, _.ret_val: Result): Lit(Result(Zero))
                    [2] Expr Block[ty=Unit]
                    [3] Expr Var[ty=Result]"#]],
    );
}

#[test]
fn early_return_in_qubit_array_scope_stays_path_local() {
    let source = indoc! {r#"
        namespace Test {
...
#[test]
fn semi_return_and_expr_return_produce_same_shape() {
