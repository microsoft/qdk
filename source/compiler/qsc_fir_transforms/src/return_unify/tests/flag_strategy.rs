...
fn while_return_tuple_value_via_flag_transform() {
    ...
    let (pat, init_expr) = find_local_init(package, "Main", symbols::RET_VAL);
...
fn all_returning_nested_if_tuple_uses_return_slot_fallback() {
    ...
    let (store, pkg_id) = compile_return_unified(source);
    let package = store.get(pkg_id);
    let (ret_val_pat, _) = find_local_init(package, "Main", symbols::RET_VAL);
    let ret_val_var_id = local_var_id_from_named_pat(ret_val_pat, symbols::RET_VAL);
    ...
    let has_trailing_result = body_block.stmts.iter().any(|stmt_id| {
        ...
        matches!(&pat.kind, PatKind::Bind(ident) if ident.name.as_ref() == symbols::TRAILING_RESULT)
    });
    assert!(
        !has_trailing_result,
...
fn assert_empty_array_return_slot(
    ...
    let (has_returned_pat, _) = find_local_init(package, callable_name, symbols::HAS_RETURNED);
    let has_returned_var_id = local_var_id_from_named_pat(has_returned_pat, symbols::HAS_RETURNED);
    let (ret_val_pat, ret_val_init) = find_local_init(package, callable_name, symbols::RET_VAL);
    let ret_val_var_id = local_var_id_from_named_pat(ret_val_pat, symbols::RET_VAL);
...
fn udt_wrapping_qubit_return_in_while_uses_array_backed_return_slot() {
    ...
    let (store, pkg_id) = compile_return_unified(source);
    let package = store.get(pkg_id);
    let (ret_val_pat, _) = find_local_init(package, "Pick", symbols::RET_VAL);
...
fn recursive_lazy_suffix_reuses_flag_pair_for_returns_inside_suffix() {
    ...
    assert_eq!(
        rendered
            .matches(&format!("mutable {} : Bool", symbols::HAS_RETURNED))
            .count(),
        1,
    ...
    assert_eq!(
        rendered
            .matches(&format!("mutable {} : Int", symbols::RET_VAL))
            .count(),
        1,
    ...
    assert!(
        rendered.contains(&format!("{} = 1;", symbols::RET_VAL))
            && rendered
                .matches(&format!("{} = true;", symbols::HAS_RETURNED))
                .count()
                >= 2,
...
fn range_return_default_in_flag_lowering_is_supported() {
    ...
    assert!(
        rendered.contains(&format!("mutable {} : Range =", symbols::RET_VAL)),
...
fn all_four_specializations_with_return_in_loop() {
    ...
    "#},
        &expect![[r#"
            ...
            operation Main() : Unit {
                let q : Qubit = __quantum__rt__qubit_allocate();
                let _generated_ident_142 : Unit = Op(q);
                __quantum__rt__qubit_release(q);
