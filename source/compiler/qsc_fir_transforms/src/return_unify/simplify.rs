...
//! # Rule ordering
//!
//! [`apply_all_rules`] runs the catalogue in this fixed order each pass:
//!
//! 1. [`guard_clause`]
//! 2. [`run_identical_branches`] (the identical-branches fold)
//! 3. [`both_branches`]
//! 4. [`single_branch`]
//! 5. [`bare_return`]
//! 6. [`let_folding`]
...
//! 8. [`dead_local`]
//!
//! The structural rules (steps 1-5) run before [`let_folding`] because
//! their patterns include the lazy `if not __has_returned` continuation
//! as a separate statement between the guard set and the merge; folding
//! that binding into the merge's else-arm first would prevent
//! [`guard_clause`] from matching. [`dead_flag`] then drops flag-set
//! assignments with no downstream reader, and [`dead_local`] runs last to
//! remove the now-unused `__has_returned` / `__ret_val` declarations.
...
/// Drive the identical-branches fold across every statement of
/// `block_id`.
///
...
///   statement of type `return_ty` (the block-wrapped slot read).
