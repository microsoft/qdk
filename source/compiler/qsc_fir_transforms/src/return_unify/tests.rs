...
fn assert_guarded_release_continuation(result: &NoHoistReturnUnifyResult, callable_name: &str) {
    let package = result.store.get(result.pkg_id);
    let release_set = collect_release_callables(&result.store);
    let (flag_pat, _) = find_local_init(package, callable_name, symbols::HAS_RETURNED);
    let has_returned_var_id = local_var_id_from_named_pat(flag_pat, symbols::HAS_RETURNED);
...
/// between the two snapshots is attributable solely to the operand lift. Use it
/// in dedicated ANF lift-shape tests so the pinned delta witnesses exactly what
/// the ANF phase rewrites, independently of the surrounding transform.
///
/// The snapshot format is
/// `// before anf (changed=<bool>)\n<fir>\n// after anf\n<fir>`. The `changed`
/// flag records the ANF driver's return value (whether any operand lift fired),
/// so the snapshot witnesses the phase firing without a separate assertion.
pub(crate) fn check_anf_isolated_q(source: &str, callable_name: &str, expect: &Expect) {
    let (mut store, pkg_id) = compile_and_run_pipeline_to(source, PipelineStage::Mono);
    let mut assigner = Assigner::from_package(store.get(pkg_id));
    let block_id = find_body_block_id(store.get(pkg_id), callable_name);

    let mut errors = Vec::new();
    super::normalize::hoist_returns_to_statement_boundary(
    ...
    let changed = super::normalize::run_anf_to_fixpoint(
        store.get_mut(pkg_id),
        &mut assigner,
        pkg_id,
        block_id,
    ...
    assert!(
        errors.is_empty(),
        "isolated hoist + ANF normalization produced errors: {errors:?}"
    );

    expect.assert_eq(&format!(
        "// before anf (changed={changed})\n{before}\n// after anf\n{after}"
    ));
}
...
pub(crate) fn check_pre_fir_transforms_to_return_unify_q(source: &str, expect: &Expect) {
...
pub(crate) fn find_local_init<'a>(
    package: &'a Package,
    callable_name: &str,
    local_name: &str,
) -> (&'a Pat, &'a Expr) {
...
pub(crate) fn find_body_block_id(package: &Package, callable_name: &str) -> BlockId {
    let decl = find_callable_decl(package, callable_name);
...
pub(crate) fn local_var_id_from_named_pat(pat: &Pat, local_name: &str) -> LocalVarId {
    let PatKind::Bind(ident) = &pat.kind else {
...
pub(crate) fn assert_while_condition_guarded_by_not_flag(
    package: &Package,
    cond_expr_id: ExprId,
    has_returned_var_id: LocalVarId,
) {
