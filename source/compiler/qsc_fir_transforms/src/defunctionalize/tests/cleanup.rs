...
/// has no live callee or argument reads is replaced with a reference to its own target
...
fn consumed_closure_outside_call_arg_is_cleaned() {
    let (mut fir_store, fir_pkg_id, reachable_item_ids) =
        setup_with_dead_callee_reads(LET_BOUND_SOURCE, &["f(q);"]);
    let package = fir_store.get(fir_pkg_id);
    let (closure_expr, target) = single_closure(package, &reachable_item_ids);
...
/// argument. It becomes eligible for cleanup only after its callee reads are gone.
///
...
fn udt_ctor_wrapped_closure_is_cleaned() {
    let (mut fir_store, fir_pkg_id, reachable_item_ids) =
        setup_with_dead_callee_reads(UDT_CTOR_SOURCE, &["(w!)(q);"]);
    let package = fir_store.get(fir_pkg_id);
    let (closure_expr, target) = single_closure(package, &reachable_item_ids);
