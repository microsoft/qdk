...
fn colliding_foreign_pattern_does_not_supply_capture_type() {
    let (mut store, package_id) = compile_to_monomorphized_fir(
        r#"
        function Foreign(foreign : Double) : Double { foreign }
        function Make(ignored : Int, own : Int) : Int -> Int { unused -> own }
        operation Main() : Int {
            let ignored = Foreign(2.0);
            let closure = Make(0, 7);
            closure(0)
        }
        "#,
    );
    let package = store.get_mut(package_id);
    let PatKind::Tuple(parameters) = &package.get_pat(callable_decl(package, "Make").input).kind
    else {
        panic!("Make should have tuple parameters");
    };
    let own_pat = package.get_pat(parameters[1]);
    let PatKind::Bind(own) = &own_pat.kind else {
        panic!("Make should bind its capture parameter");
    };
    let colliding_var = own.id;
    assert_eq!(own_pat.ty, Ty::Prim(Prim::Int));
    assert!(package.exprs.values().any(|expr| {
        matches!(&expr.kind, fir::ExprKind::Closure(captures, _) if captures.as_slice() == [colliding_var])
    }), "Make should capture its own parameter before corruption");

    let foreign_pat_id = callable_decl(package, "Foreign").input;
    let foreign_pat = package
        .pats
        .get_mut(foreign_pat_id)
        .expect("Foreign input should exist");
    assert_eq!(foreign_pat.ty, Ty::Prim(Prim::Double));
    let PatKind::Bind(foreign) = &mut foreign_pat.kind else {
        panic!("Foreign should bind its own parameter");
    };
    assert_ne!(foreign.id, colliding_var);
    foreign.id = colliding_var;
    let locals = LocalState::default();
