...
/// pattern contains nested tuples or discards that cannot be mapped to
    ...
    match &pat.kind {
        ...
        PatKind::Tuple(sub_pats) => {
            ...
            for &sub_pat_id in sub_pats {
                ...
                match &sub_pat.kind {
                    PatKind::Bind(ident) => vars.push(ident.id),
                    _ => return None,
                }
            }
            Some(vars)
...
fn args_forward_params_in_order(
    ...
    if actual_param_vars.len() == 1 {
        matches!(
            &args_expr.kind,
            ExprKind::Var(Res::Local(var), _) if *var == actual_param_vars[0]
        )
    } else if let ExprKind::Tuple(elements) = &args_expr.kind {
        elements.len() == actual_param_vars.len()
            && elements
                .iter()
                .zip(actual_param_vars)
                .all(|(&elem_id, &expected_var)| {
                    let elem_expr = pkg.get_expr(elem_id);
                    matches!(
                        &elem_expr.kind,
                        ExprKind::Var(Res::Local(var), _) if *var == expected_var
                    )
                })
    } else {
        false
...
fn captures_appear_in_args(
    ...
    match &args_expr.kind {
        ExprKind::Var(Res::Local(var), _) => capture_param_vars.contains(var),
        ExprKind::Tuple(elements) => elements.iter().any(|&elem_id| {
            let elem_expr = pkg.get_expr(elem_id);
            matches!(
                &elem_expr.kind,
                ExprKind::Var(Res::Local(var), _) if capture_param_vars.contains(var)
            )
        }),
