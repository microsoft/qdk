...
/// Descends into nested expressions to find the tuple field path of a local
/// variable binding.
///
/// Exhaustive over every `ExprKind` that holds nested expressions (mirroring
/// [`find_local_init_expr_in_expr`]) so a tuple-pattern dispatch binding in an
/// operand-position block (for example `let z = { let (_, callee, _) = arr[i];
/// callee(q) } + 1`) is still found, rather than dropped by a wildcard arm.
fn find_var_tuple_field_path_in_expr(
    ...
    match &expr.kind {
        ExprKind::Array(exprs) | ExprKind::ArrayLit(exprs) | ExprKind::Tuple(exprs) => exprs
            .iter()
            .find_map(|&expr_id| find_var_tuple_field_path_in_expr(package, expr_id, local_var)),
        ExprKind::ArrayRepeat(lhs, rhs)
        | ExprKind::Assign(lhs, rhs)
        | ExprKind::AssignOp(_, lhs, rhs)
        | ExprKind::BinOp(_, lhs, rhs)
        ...
        | ExprKind::AssignField(lhs, _, rhs)
        | ExprKind::UpdateField(lhs, _, rhs) => {
            find_var_tuple_field_path_in_expr(package, *lhs, local_var)
                .or_else(|| find_var_tuple_field_path_in_expr(package, *rhs, local_var))
        }
        ExprKind::AssignIndex(a, b, c) | ExprKind::UpdateIndex(a, b, c) => {
            find_var_tuple_field_path_in_expr(package, *a, local_var)
                .or_else(|| find_var_tuple_field_path_in_expr(package, *b, local_var))
                .or_else(|| find_var_tuple_field_path_in_expr(package, *c, local_var))
        }
        ExprKind::Block(block_id) => {
        ...
        | ExprKind::Field(inner, _)
        | ExprKind::Return(inner)
        | ExprKind::UnOp(_, inner) => find_var_tuple_field_path_in_expr(package, *inner, local_var),
        ExprKind::If(cond, body, otherwise) => {
            find_var_tuple_field_path_in_expr(package, *cond, local_var)
                .or_else(|| find_var_tuple_field_path_in_expr(package, *body, local_var))
                .or_else(|| {
                    otherwise.and_then(|e| find_var_tuple_field_path_in_expr(package, e, local_var))
                })
        }
        ExprKind::Range(start, step, end) => start
            .and_then(|e| find_var_tuple_field_path_in_expr(package, e, local_var))
            .or_else(|| step.and_then(|e| find_var_tuple_field_path_in_expr(package, e, local_var)))
            .or_else(|| end.and_then(|e| find_var_tuple_field_path_in_expr(package, e, local_var))),
        ExprKind::String(components) => components.iter().find_map(|component| match component {
            qsc_fir::fir::StringComponent::Expr(expr_id) => {
                find_var_tuple_field_path_in_expr(package, *expr_id, local_var)
            }
        ...
        ExprKind::Struct(_, copy, fields) => copy
            ...
            .or_else(|| {
                fields.iter().find_map(|field| {
                    find_var_tuple_field_path_in_expr(package, field.value, local_var)
                })
            }),
        ExprKind::While(cond, block_id) => {
            find_var_tuple_field_path_in_expr(package, *cond, local_var)
