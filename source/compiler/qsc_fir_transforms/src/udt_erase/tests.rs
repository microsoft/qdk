...
use qsc_fir::fir::{
    ...
    FieldPath, Ident, Item, ItemId, LocalVarId, PackageLookup, Pat, PatKind, SpecDecl, SpecImpl,
    Stmt, StmtId, StmtKind, Visibility,
};
    ...
    check_before_after_udt_erase(
        ...
        &expect![[r#"
            ...
            function MakeOuter() : __UDT_Item_2__Package_2_ {
                let i : __UDT_Item_1__Package_2_ = Inner(1, 2);
                Outer(i, true)
            }
            function Main() : Unit {
                let _ : __UDT_Item_2__Package_2_ = MakeOuter();
            }
            // entry
            Main()
...
fn udt_as_callable_return_type() {
    ...
    check_before_after_udt_erase(
        ...
        &expect![[r#"
            ...
            function Main() : Unit {
                let _ : __UDT_Item_1__Package_2_ = MakeIt();
            }
            // entry
            Main()
...
fn three_level_nested_udt_fully_erased() {
    ...
    check_before_after_udt_erase(
        ...
        &expect![[r#"
            ...
            function Main() : Int {
                let o : __UDT_Item_3__Package_2_ = new Outer {
                    M = new Middle {
                        I = new Inner {
                            X = 42
                        },
            ...
            newtype Outer = (__UDT_Item_2__Package_2_, Bool);
            function Main() : Int {
                let o : (((Int, ), Double), Bool) = (((42, ), 1.), true);
                o::Item < 0 >::Item < 0 >::Item < 0 >
            }
            // entry
            Main()
        "#]],
...
fn udt_as_callable_return_type_erased() {
    ...
    check_before_after_udt_erase(
        ...
        &expect![[r#"
            ...
            function Main() : Int {
                let p : __UDT_Item_1__Package_2_ = MakePair(1, 2.);
                p::Fst
            }
            // entry
            Main()
...
fn post_udt_erase_invariants_cover_reachable_external_packages() {
    ...
    crate::test_utils::assert_panics_with("contains Ty::Udt after UDT erasure", || {
        crate::invariants::check(
            &store,
            target_pkg_id,
            crate::invariants::InvariantLevel::PostUdtErase,
...
fn single_field_struct_return_type_erased_to_single_element_tuple() {
    ...
    check_before_after_udt_erase(
        ...
        &expect![[r#"
            ...
            function Main() : Unit {
                let _ : __UDT_Item_1__Package_2_ = Make();
            }
            // entry
            Main()
