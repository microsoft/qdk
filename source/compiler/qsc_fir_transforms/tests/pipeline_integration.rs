...
// ============================================================================
// These tests verify that FIR output at a given pipeline stage has the same
// reachable callable surface as the full pipeline. `assert_stage_parity` checks
// two views for parity against a full run:
//
// 1. Reachable callable names (callables are not unexpectedly added or removed)
// 2. Reachable callable type summary (callable types match across the boundary)
//
// The staged output is additionally invariant-checked and the full run is
