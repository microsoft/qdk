    ...
    check_qasm_to_qsharp(
        ...
        &expect![[r#"
            ...
            Qasm.Compiler.NotSupported
              ...
              | supported
                ,-[Test.qasm:40:24]
             39 | // NotSupported mutable array reference
             40 | def mut_subroutine_dyn(mutable array[int[8], #dim = 1] arr_arg) {
                :                        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
             41 | 
                `----
            ...
            Qasm.Compiler.NotSupported
              ...
              x mutable array references `mutable array[int[8], 2, 3]` are not supported
                ,-[Test.qasm:45:27]
             44 | // NotSupported mutable static sized array reference
             45 | def mut_subroutine_static(mutable array[int[8], 2, 3] arr_arg) {
                :                           ^^^^^^^^^^^^^^^^^^^^^^^^^^^
             46 | 
                `----
            ...
            Qasm.Compiler.NotSupported
              ...
              x hardware qubit operands are not supported
                ,-[Test.qasm:72:3]
             71 | // NotSupported hardware qubit
             72 | x $0;
                :   ^^
                `----
        "#]],
