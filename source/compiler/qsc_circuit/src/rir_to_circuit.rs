...
use crate::{
    Circuit, Error, Operation, TracerConfig,
    builder::{
    ...
    circuit::{Ket, Measurement, Metadata, Register, Unitary},
...
pub fn make_circuit(
    ...
    let circuit = finish_circuit(source_lookup, operations, qubits, num_qubits);
...
fn extend_block_with_branch_instruction_vars_only(
    ...
    let metadata = instruction_metadata
        ...
        .map(|span| PackageOffset {
            package_id: span.package_id.into(),
            offset: span.offset,
        });
    let branch = Branch {
        condition: variable,
