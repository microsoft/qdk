...
impl ExecGraphBuilder {
    ...
    fn debug_set(&mut self, index: ExecGraphIdx, node: ExecGraphNode) {
        self.debug[index.debug_idx] = node;
    }
    ...
    fn lower_expr(&mut self, expr: &hir::Expr) -> ExprId {
        ...
        let kind = match &expr.kind {
            ...
            hir::ExprKind::While(cond, body) => {
                ...
                // Placeholder
                self.exec_graph
                    .debug_push(ExecGraphDebugNode::PushLoopScope(0u32.into()));

                let cond_idx = self.exec_graph.len();
                let cond = self.lower_expr(cond);

                self.exec_graph.debug_set(
                    loop_start_idx,
                    ExecGraphNode::Debug(ExecGraphDebugNode::PushLoopScope(cond)),
                );
                ...
                let next_block_idx = self.exec_graph.len();
                self.exec_graph
                    .set_with_arg(ExecGraphNode::JumpIfNot, jump_idx, next_block_idx);
