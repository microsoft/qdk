...
pub enum Error {
    ...
    #[diagnostic(code("Stim.MeasurementRecordOutOfPrepareScope"))]
    MeasurementRecordOutOfPrepareScope {
        #[label]
        span: Span,
    },
...
impl<'noise> Compiler<'noise> {
    ...
    fn compile_block(&mut self, block: &Block) {
        ...
        self.enter_block(block_instruction);
        for item in items {
            self.compile_item(item);
        }
        self.exit_block(block_instruction);
    }

    fn enter_block(&mut self, block_instruction: &Instruction) {
        match block_instruction.name.as_str() {
            "PREPARE" => {
                self.id_map.enter_prepare_scope();
            }
            "REPEAT" => (),
            _ => self.unknown(block_instruction),
        }
        self.compile_instruction(block_instruction);
    }

    fn exit_block(&mut self, block_instruction: &Instruction) {
        match block_instruction.name.as_str() {
            "PREPARE" => {
    ...
    fn resolve_record_offset(&mut self, target: &Target, offset: u32) -> Option<u32> {
        ...
        if self.id_map.scope_of_record(result_id) != self.id_map.current_scope() {
            self.push_error(Error::MeasurementRecordOutOfPrepareScope { span: target.span });
            return None;
        }
        Some(result_id)
    }
