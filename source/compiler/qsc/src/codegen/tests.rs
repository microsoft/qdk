    ...
    let source = indoc::indoc! {r#"
        namespace Repro {
            ...
            operation SelectSwap(
                ...
                let addressFitted = address[...Length(address) - 1];
                ...
                if numSwapBits == 0 {
                    ...
                    WithSelectSwap(
                        ...
                        data,
                        address,
                        intermediate => ApplyToEachCA(
                            CNOT,
                            Zipped(intermediate, output),
                        ),
                    );
                }
            }

            operation WithSelectSwap(
                numSwapBits : Int,
                data : Bool[][],
                address : Qubit[],
                action : (Qubit[] => Unit is Adj + Ctl),
            ) : Unit is Adj + Ctl {
                body (...) {
                    let m = Length(data[0]);
                    let addressParts = Partitioned(
                        [Length(address) - numSwapBits, numSwapBits],
                        address,
                    );

                    use dataRegister = Qubit[m * 2^numSwapBits];
                    let outputs = Chunks(m, dataRegister);

                    within {
                        Select(data, addressParts[0], dataRegister);
                    } apply {
                        action(outputs[0]);
                    }
                }
            ...
            operation Invoke() : Unit {
                use address = Qubit[1];
                use output = Qubit[1];
                SelectSwap(0, [[false], [true]], address, output);
            }
