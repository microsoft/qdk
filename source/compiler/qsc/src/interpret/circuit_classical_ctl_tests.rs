...
fn result_comparison_to_literal() {
    ...
    expect![[r#"
        ...
        [1] Main:
            q_0    ─ H@test.qs:2:4 ── M@test.qs:3:13 ─── X@test.qs:5:8 ─── |0〉@test.qs:7:4 ──
                                             ╘════════════════ ● ════════════════════════════
    "#]]
    .assert_eq(&circ);
}
...
fn else_block_only() {
    ...
    expect![[r#"
        ...
        [1] Main:
            q_0    ─ H@test.qs:2:4 ── M@test.qs:3:13 ─── X@test.qs:6:8 ─── |0〉@test.qs:8:4 ──
                                             ╘════════════════ ● ════════════════════════════
    "#]]
    .assert_eq(&circ);
}
    ...
    expect![[r#"
        ...
        [1] Main:
            q_0    ─ H@test.qs:3:4 ── M@test.qs:4:12 ───────────────────────────────────────────────────────
                                             ╘════════════════ ● ══════════════ ○ ══════════════════════════
            q_1    ───────────────────────────────────── X@test.qs:6:8 ── Y@test.qs:8:8 ── M@test.qs:10:13 ─
                                                                                                  ╘═════════
    "#]]
    .assert_eq(&circ);
}
    ...
    expect![[r#"
        ...
        [1] Main:
            ...
            q_1    ─ H@test.qs:5:4 ── M@test.qs:7:13 ──────────┼─────────────────┼──────────────────────────────────────────────────────────────────
                                             ╘═════════════════╪═════════════════╪═════════════════ ● ════════════════ ○ ═══════════════════════════
            q_2    ───────────────────────────────────── X@test.qs:9:8 ── Z@test.qs:11:8 ─── X@test.qs:14:8 ─── Y@test.qs:16:8 ─── M@test.qs:18:13 ─
                                                                                                                                          ╘═════════
    "#]]
    .assert_eq(&circ);
}
...
fn nested_ifs() {
    ...
    expect![[r#"
        ...
        [1] Main:
            ...
            q_1    ─ H@test.qs:5:4 ── M@test.qs:7:13 ───────────────────┼──────────────────────────┼────────────────────────────
                                             ╘══════════════════════════╪══════════════════════════╪════════════════════════════
            q_2    ────────────────────────────────────── if: c_0 = |1〉@test.qs:8:4[2] ──── Z@test.qs:15:8 ─── M@test.qs:17:13 ─
                                                                                                                      ╘═════════
        ...
        [2] if: c_0 = |1〉:
            ...
            q_1    ──────────────────────────────────────
                            ●                  ○
            q_2    ─ X@test.qs:10:12 ── Y@test.qs:12:12 ─
...
fn nested_callables_in_branch() {
    ...
    expect![[r#"
        ...
        [1] Main:
            q_0    ─ [ [Foo@test.qs:2:4] ── [ [Bar@test.qs:10:4] ─── X@test.qs:13:4 ─── Y@test.qs:14:4 ──── ] ──── ] ──────────────────── Foo@test.qs:6:8[2] ──
                                                                                                                                                   ┆
            q_1    ──── H@test.qs:4:4 ────────────────────────────────────────────────────────────────────────────────── M@test.qs:5:8 ────────────┆───────────
                                                                                                                               ╘═══════════════════┆═══════════

        [2] Foo:
            q_0    ─ Bar@test.qs:10:4[3] ─
                              ┆
            q_1    ───────────┆───────────
                              ╘═══════════

        [3] Bar:
            q_0    ─ X@test.qs:13:4 ─── Y@test.qs:14:4 ──
            q_1    ─────────┼──────────────────┼─────────
                            ●                  ●
    "#]]
    .assert_eq(&circ);
