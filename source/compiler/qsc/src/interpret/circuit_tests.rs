...
// `prepare_codegen_fir_from_callable_args` -> `CallableArgsBackend::SyntheticEntry`,
// exercising the `static_circuit_from_callable` SyntheticEntry arm. Parity is
// asserted with `source_locations: false` because qubit-allocation source spans
// are not currently represented in the Static method (matching the existing
// `circuit_without_groups` harness constraint).
#[test]
fn static_circuit_from_callable_with_callable_arg_matches_classical_eval() {
    let code = r#"
        namespace Test {
    ...
    "#;
    let mut interp = interpreter(code, PackageType::Lib, Profile::AdaptiveRIF);
    interp.set_quantum_seed(Some(2));
    ...
    let static_circ = interp
        .circuit(
            CircuitEntryPoint::Callable(invoke.clone(), args.clone()),
            CircuitGenerationMethod::Static,
            cfg,
        )
        .expect("static circuit generation should succeed");
    let eval_circ = interp
        .circuit(
            CircuitEntryPoint::Callable(invoke, args),
            CircuitGenerationMethod::ClassicalEval,
            cfg,
        )
        .expect("classical-eval circuit generation should succeed");

    // Regression lock: the Static path must consume the synthetic entry so
    // its circuit matches the ClassicalEval circuit for SyntheticEntry-routed args.
    assert_eq!(static_circ.to_string(), eval_circ.to_string());
    ...
    expect![[r#"
        ...
        q_2    ── H ──
    "#]]
    .assert_eq(&static_circ.to_string());
}
...
fn dynamic_for_loop_is_grouped() {
    ...
    expect![[r#"
        ...
        [1] Main:
            q_0@test.qs:2:15 ─ loop: M(qubit) == Zero@test.qs:3:16[2] ──
                                                  ╘═════════════════════
                                                  ╘═════════════════════
                                                  ╘═════════════════════
                                                  ╘═════════════════════
        ...
        [2] loop: M(qubit) == Zero:
            q_0@test.qs:2:15 ─ (1)@test.qs:3:23[3] ── (2)@test.qs:3:23[4] ── (3)@test.qs:3:23[5] ── (4)@test.qs:3:23[6] ─
                                        ╘══════════════════════┆══════════════════════┆══════════════════════┆═══════════
                                                               ╘══════════════════════┆══════════════════════┆═══════════
                                                                                      ╘══════════════════════┆═══════════
                                                                                                             ╘═══════════
        ...
        [3] (1):
            q_0@test.qs:2:15 ─ H@test.qs:4:20 ─── M@test.qs:5:24 ──── |0〉@test.qs:7:20 ───
                                                         ╘════════════════════════════════
        ...
        [6] (4):
            q_0@test.qs:2:15 ─ H@test.qs:4:20 ─── M@test.qs:5:24 ──
                                                         │
                                                         │
                                                         │
                                                         ╘═════════
...
fn loop_vertical_is_not_grouped() {
    ...
    expect![[r#"
        ...
        q_2@test.qs:4:20 ─ Main[1] ─
                              ┆
        q_3@test.qs:4:20 ─ Main[1] ─
                              ┆
        q_4@test.qs:4:20 ─ Main[1] ─
                              ┆
        q_5@test.qs:4:20 ─ Main[1] ─

        [1] Main:
            q_0@test.qs:4:20 ─ [ [Foo@test.qs:6:24] ─── X@test.qs:11:20 ─── ] ──
            q_1@test.qs:4:20 ─ [ [Foo@test.qs:6:24] ─── X@test.qs:11:20 ─── ] ──
            q_2@test.qs:4:20 ─ [ [Foo@test.qs:6:24] ─── X@test.qs:11:20 ─── ] ──
            q_3@test.qs:4:20 ─ [ [Foo@test.qs:6:24] ─── X@test.qs:11:20 ─── ] ──
            q_4@test.qs:4:20 ─ [ [Foo@test.qs:6:24] ─── X@test.qs:11:20 ─── ] ──
            q_5@test.qs:4:20 ─ [ [Foo@test.qs:6:24] ─── X@test.qs:11:20 ─── ] ──
    "#]]
    .assert_eq(&circ);
}
