...
fn nested_callables() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main::Foo::Bar] ──── X ──── Y ─── ]] ─── [[ ─── [Main::Bar] ─── X ──── Y ─── ]] ─── [[ ─── [Main::Bar] ─── X ──── Y ─── ]] ──
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn nested_callables_with_measurement() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main::Foo::Bar] ──── X ──── Y ─── ]] ──── M ──── |0〉 ─── [[ ─── [Main::Bar] ─── X ──── Y ─── ]] ──── M ──── |0〉 ─── [[ ─── [Main::Bar] ─── X ──── Y ─── ]] ──── M ──── |0〉 ──
                                                                   ╘══════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════╪════════════
                                                                                                                              ╘══════════════════════════════════════════════════════════╪════════════
                                                                                                                                                                                         ╘════════════
    "#]]
...
fn result_comparison_to_literal() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
                                                  ╘═════════════════════ ● ══════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn result_comparison_to_literal_zero() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─── [[ ──── [check (c_0 = |0〉)] ─── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
                                                  ╘═════════════════════ ● ══════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn else_block_only() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─── [[ ──── [check (c_0 = |0〉)] ─── [[ ─── [false] ── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
                                                  ╘═════════════════════ ● ══════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn result_comparison_to_result() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─── [[ ───── [check (c_0c_1 = |00〉 or c_0c_1 = |11〉)] ───── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
                           ┆                      ╘═════════════════════════════════ ● ══════════════════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════
        q_1    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─────────────────────────────────┼────────────────────────────────────┼──────────────────────────────────────────────────── [[ ─── [Main] ──── |0〉 ─── ]] ──
                                                  ╘═════════════════════════════════ ● ══════════════════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════
    "#]]
...
fn loop_and_scope() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main::PrepareSomething] ─── [[ ─── [H X X(×10)] ──── H ──── X ──── ● ─── [[ ──── [H(×9)] ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main::DoSomethingElse] ── [[ ──── [H X X...(×10)] ──── X ─────────── X ─── [[ ──── [H(×9)] ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ──── X ─────────── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main::DoSomethingDifferent] ─── [[ ─── [H Z X(×10)] ──── H ──── Z ──── ● ─── [[ ──── [H(×9)] ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ──── H ──── Z ──── ● ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
                                    ┆                             ┆                         │                ┆                       │                    │                    │                    │                    │                    │                    │                    │                    │                                            ┆                              ┆                           │                ┆                       │                    │                    │                    │                    │                    │                    │                    │                    │                                               ┆                               ┆                         │                ┆                       │                    │                    │                    │                    │                    │                    │                    │                    │
        q_1    ─ [[ ─── [Main::PrepareSomething] ─── [[ ─── [H X X(×10)] ────────────────── X ─── [[ ──── [H(×9)] ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main::DoSomethingElse] ── [[ ──── [H X X...(×10)] ──── H ──── X ──── ● ─── [[ ──── [H(×9)] ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ──── H ──── X ──── ● ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main::DoSomethingDifferent] ─── [[ ─── [H Z X(×10)] ────────────────── X ─── [[ ──── [H(×9)] ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ────────────────── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn operation_with_long_gates_properly_aligned() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ────────────────────────── ● ─── ]] ──── M ─── [[ ─── [Main] ───────────────────────────────────────────────────────────────────────────────────── ● ─── ]] ─────────
                           ┆                                     │             ╘═══════════════┆═════════════════════════════════════════════════════════════════════════════════════════╪═════════════════
        q_1    ─ [[ ─── [Main] ──── H ──── X ─── Ry(1.0000) ──── X ─── ]] ────────── [[ ─── [Main] ───────────────────────────────────────────────────────────────────── Rxx(1.0000) ────┼──── ]] ──── M ──
                                                                                               ┆                                                                              ┆          │             ╘═══
...
fn operation_with_subsequent_qubits_gets_horizontal_lines() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ──
                               ┆                ┆
        q_1    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ──
                               ┆
        q_2    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ──
                               ┆                ┆
        q_3    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ──
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn operation_with_subsequent_qubits_no_double_rows() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── Rxx(1.0000) ── ]] ──
                               ┆                ┆              ┆
        q_1    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── Rxx(1.0000) ── ]] ──
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn operation_with_subsequent_qubits_no_added_rows() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ──── M ──
                               ┆                ┆                 ╘═══
        q_1    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ─────────
                               ┆
        q_2    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ──── M ──
                               ┆                ┆                 ╘═══
        q_3    ─ [[ ─── [Main(1.0000)] ─── Rxx(1.0000) ── ]] ─────────
    "#]]
    .assert_eq(&circ.to_string());
...
fn if_else() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                                                  ╘═════════════════════ ● ══════════════════════ ● ══════════════════════════════════════════════════════ ● ═════════════════════════════════════════════════════════
        q_1    ──────────────────────────────────────── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── [[ ─── [false] ── [[ ─── [Main] ──── Y ─── ]] ─── ]] ─── ]] ──── M ──
                                                                                                                                                                                                                  ╘═══
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn sequential_ifs() {
    ...
    expect![[r#"
        ...
        q_1    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─────────────────────┼────────────────────────┼────────────────────────────────────────────────────────┼───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                                                  ╘══════════════════════╪════════════════════════╪════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ══════════════════════════════════════════════════════ ● ═════════════════════════════════════════════════════════
        q_2    ──────────────────────────────────────── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── [[ ─── [false] ── [[ ─── [Main] ──── Z ─── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_1 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── [[ ─── [false] ── [[ ─── [Main] ──── Y ─── ]] ─── ]] ─── ]] ──── M ──
                                                                                                                                                                                                                                                                                                                                                                           ╘═══
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn multiple_possible_float_values_in_unitary_arg() {
    ...
    expect![[r#"
        q_0    ─ [[ ───────────── [Main] ──────────────────────── H ───────────── ]] ──── M ──
                                                     ════════════ ● ════════════          ╘═══
        q_1    ─ [[ ─── [Main(function of: (c_0))] ─── Rx(function of: (c_0)) ─── ]] ──── M ──
                                                                                          ╘═══
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn custom_intrinsic_variable_arg() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─── [[ ─── [Main(function of: (c_0))] ─── foo(function of: (c_0)) ── ]] ──
                                                  ╘═════════════════════════════════════════════════════ ● ═══════════════════
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn branch_on_dynamic_bool() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──── H ─── ]] ──── M ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                                                  ╘═════════════════════════ ● ══════════════════════════ ● ═════════════════════════════════════════════════════════
        q_1    ──────────────────────────────────────── [[ ─── [check (function of: (c_0))] ─── [[ ─── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── ]] ──── M ──
                                                                                                                                                                 ╘═══
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn teleportation() {
    ...
    expect![[r#"
        q_0    ─ [[ ─── [Main] ──────── H ──────── ● ──── X ────────── ]] ──── M ─── [[ ────────── [Main] ─────────── |0〉 ───── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                           ┆                       │      │                    ╘═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ════════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
        q_1    ─ [[ ─── [Main] ─────────────────── X ─────┼─────────── ]] ────────── [[ ──── [check (c_0 = |1〉)] ──── [[ ──── [true] ─── [[ ─── [Main] ──── Z ─── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_1 = |1〉)] ──── [[ ──── [true] ─── [[ ─── [Main] ──── X ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main(-0.7000)] ── Rx(-0.7000) ── ]] ─── [[ ─── [Main] ──── |0〉 ─── ]] ──
                           ┆                              │                                           │                          │
        q_2    ─ [[ ─── [Main] ─── Rx(0.7000) ─────────── ● ──── H ─── ]] ──── M ─────────────────────┼──────────────────────────┼──────────────────────────────────────────────────── [[ ────────── [Main] ─────────── |0〉 ───── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                                                                               ╘═════════════════════ ● ════════════════════════ ● ══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
    "#]]
    .assert_eq(&circ.to_string());
}
...
fn dot_product_phase_estimation() {
    ...
    .expect("circuit generation should succeed");
    expect![[r#"
        q_0    ─ [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::StateInitialisation] ── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(-0.3142) ─── X ─── Rz(0.3142) ──── X ──── H ──── S ─── S' ──── H ─── Rz(-0.2244) ─── X ─── Rz(0.2244) ──── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ──── H ──── X ──── H ──── X ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ─── S' ──── H ─── Rz(0.2244) ──── X ─── Rz(-0.2244) ─── X ──── H ──── S ─── S' ──── H ─── Rz(0.3142) ──── X ─── Rz(-0.3142) ─── X ──── H ──── S ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct] ──── |0〉 ─── ]] ───────────
                                                                       ┆                                                                                 │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                 ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ┆                                                                                      │                     │                                                 │                     │                                                                          ┆                                                            │                                                                                     ┆                                                                                      │                     │                                                 │                     │
        q_1    ─ [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::StateInitialisation] ─── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── Z ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ──── X ─────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── H ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── X ─────────── ● ──── X ────────── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle::StateInitialisation] ──── H ────────────────────────── ● ─────────────────── ● ──── X ──────────────────────────────────────── ● ─────────────────── ● ──── X ──── H ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct] ──── |0〉 ─── ]] ───────────
                                                                                                                                                                                                                                                                                                                                  ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                     ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         ┆                                              │                                                                                                                                                                                                                                                                                                                                       ┆                                                            │
        q_2    ─ [[ ───────────── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ────────────── H ─── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ─── [[ ────────────────── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ─────────────────── H ─── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── M ──── |0〉 ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ──── H ─── ]] ─── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-1.5708)] ── Rz(-1.5708) ── ]] ─── ]] ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ─── [[ ────────────────── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ─────────────────── H ─── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── M ──── |0〉 ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ──── H ─── ]] ─── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-0.7854)] ── Rz(-0.7854) ── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_1 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-1.5708)] ── Rz(-1.5708) ── ]] ─── ]] ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ─── [[ ────────────────── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ─────────────────── H ─── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── M ──── |0〉 ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ──── H ─── ]] ─── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-0.3927)] ── Rz(-0.3927) ── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_1 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-0.7854)] ── Rz(-0.7854) ── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_2 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-1.5708)] ── Rz(-1.5708) ── ]] ─── ]] ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ─── [[ ────────────────── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ─────────────────── H ─── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── M ──── |0〉 ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ──── H ─── ]] ─── [[ ──── [check (c_0 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-0.1963)] ── Rz(-0.1963) ── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_1 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-0.3927)] ── Rz(-0.3927) ── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_2 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-0.7854)] ── Rz(-0.7854) ── ]] ─── ]] ─── ]] ─── [[ ──── [check (c_3 = |1〉)] ─── [[ ─── [true] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation(-1.5708)] ── Rz(-1.5708) ── ]] ─── ]] ─── ]] ─── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ─── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation::GOracle] ───────────────── ● ───────────────── ]] ─── [[ ────────────────── [Main::PerformMeasurements::QuantumInnerProduct::IterativePhaseEstimation] ─────────────────── H ─── ]] ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── M ───────────────────────────────────────────────────────────────────────── |0〉 ──
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   ╘══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════════════════
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           ╘══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════════════════
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               ╘══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪════════════════════════╪════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════════════════
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   ╘══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════ ● ═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╪═════════════════════════════════════════════════════════════════════════════════
...
fn dynamics_small() {
    ...
    .expect("circuit generation should succeed");
    expect![[r#"
        q_0    ─ [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.3730)] ─── Rx(0.3730) ─── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.7461)] ─── Rx(0.7461) ─── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(-0.2191)] ── Rx(-0.2191) ── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.5922)] ── Rzz(0.5922) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.5922)] ── Rzz(0.5922) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(-0.2191)] ── Rx(-0.2191) ── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.7461)] ─── Rx(0.7461) ─── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.7461)] ─── Rx(0.7461) ─── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.7461)] ─── Rx(0.7461) ─── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(-0.2191)] ── Rx(-0.2191) ── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.5922)] ── Rzz(0.5922) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.5922)] ── Rzz(0.5922) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(-0.2191)] ── Rx(-0.2191) ── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.7461)] ─── Rx(0.7461) ─── ]] ───────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyDoubleZ(0.7461)] ── Rzz(0.7461) ── ]] ─── [[ ─── [Main::IsingModel2DSim::ApplyAllX::<lambda>(0.3730)] ─── Rx(0.3730) ─── ]] ──
                                                  ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆                                                                                                                                                               ┆                               ┆                                             ┆                               ┆                                                 ┆
...
fn xqpe() {
    let circ = circuit(XQPE, CircuitEntryPoint::EntryPoint, Config::default())
        .expect("circuit generation should succeed");
    expect![[r#"
        q_0    ─ [[ ──── [Main::IQPEMSB::IQPEMSBIteration] ──── H ─── ]] ─────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ──── H ──── ]] ──── M ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── |0〉 ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ─── H ─── ]] ─── [[ ─── [check (function of: (c_0))] ─── [[ ─── [true] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration(function of: (c_0))] ── Rz(function of: (c_0)) ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ──── H ──── ]] ──── M ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── |0〉 ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ─── H ─── ]] ─── [[ ─── [check (function of: (c_0, c_1))] ── [[ ─── [true] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration(function of: (c_0, c_1))] ─── Rz(function of: (c_0, c_1)) ── ]] ─── ]] ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ──── H ──── ]] ──── M ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── |0〉 ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ─── H ─── ]] ─── [[ ─── [check (function of: (c_0, c_1, c_2))] ─── [[ ─── [true] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration(function of: (c_0, c_1, c_2))] ── Rz(function of: (c_0, c_1, c_2)) ─── ]] ─── ]] ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ───────────────────────────────── ● ─────────────────── ● ──────────────────────────────────────── ● ─────────────────── ● ────────── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration] ──── H ──── ]] ──── M ──── |0〉 ──
                                                                                                                                                                                                                           ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                ╘═════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════════ ● ════════════════════════════════════════════════════════════════════════════════ ● ═════════════════════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪═════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪═════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪═════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪════════════════════════════════════════════════════════════════════════════════╪═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ════════════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════════ ● ═══════════════════════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪═════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═══════════════════════════════ ● ═══════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪════════════════════════════════════════════════════════════════════════════════╪════════════
                                                                                                                                                                                                                           ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                ╘═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ════════════════════════════ ● ════════════════════════════════════════════════════════════════════════════════════════ ● ═══════════════════════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪═════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪════════════════════════════════════════════════════════════════════════════════╪══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═══════════════════════════════ ● ═══════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪════════════════════════════════════════════════════════════════════════════════╪════════════
                                                                                                                                                                                                                           ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                ╘══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════ ● ═══════════════════════════════ ● ═══════════════════════════════════════════════════════════════════════════════════════════════ ● ══════════════════════════════════════════════════════════════════════════┆═════════════════════════════════════════════════════════════╪═════════════════════╪══════════════════════════════════════════╪═════════════════════╪════════════════════════════════════════════════════════════════════════════════╪════════════
                                                                                                                                                                                                                           ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    ┆                                                             │                     │                                          │                     │                                                     ┆                                                             │                     │                                          │                     │                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    ┆                                                             │                     │                                          │                     │                                                                                ╘════════════
        q_1    ─ [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ─── Rz(1.2870) ──── H ──── S ──── ● ─────────────────── ● ───────────────────────────────── ● ──── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ─── [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ─── Rz(1.2870) ──── H ──── S ──── ● ─────────────────── ● ───────────────────────────────── ● ──── ● ─── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ─── [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ─── Rz(1.2870) ──── H ──── S ──── ● ─────────────────── ● ───────────────────────────────── ● ──── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ─── [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ─── Rz(1.2870) ──── H ──── S ──── ● ─────────────────── ● ───────────────────────────────── ● ──── ● ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ──── H ──── X ─── Rz(1.5708) ──── X ─── Rz(-1.5708) ─── X ──── X ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ──────────────────
                                         ┆                                                                 │                     │                                   │      │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                                                                                ┆                                                                 │                     │                                   │      │                                                                                                                                                                                                                                                                                           ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                                                                                ┆                                                                 │                     │                                   │      │                                                                                                                                                                                                                                                                                                         ┆                                       │                                                  │             │                                                  │                                              ┆                                       │                                                  │             │                                                  │                                                                                                ┆                                                                 │                     │                                   │      │                                                                                                                                                                                                                                                                                                                         ┆                                       │                                                  │             │                                                  │
        q_2    ─ [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ───────────────────────────────── X ─── Rz(-1.5708) ─── X ─── Rz(1.5708) ──── H ──── S ──── X ──── X ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ─── [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ───────────────────────────────── X ─── Rz(-1.5708) ─── X ─── Rz(1.5708) ──── H ──── S ──── X ──── X ─── ]] ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ─── [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ───────────────────────────────── X ─── Rz(-1.5708) ─── X ─── Rz(1.5708) ──── H ──── S ──── X ──── X ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ─── [[ ─── [Main::IQPEMSB::PrepareSparseState] ── S' ──── H ───────────────────────────────── X ─── Rz(-1.5708) ─── X ─── Rz(1.5708) ──── H ──── S ──── X ──── X ─── ]] ────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── [[ ─── [Main::IQPEMSB::IQPEMSBIteration::ControlledEvolution] ──── H ──── ● ──────────────────────────────────────────────── ● ──── H ──── ● ──────────────────────────────────────────────── ● ─── ]] ─── [[ ──────────── [Main::IQPEMSB] ──────────── |0〉 ─── ]] ──────────────────
    "#]].assert_eq(&circ.to_string());
}
