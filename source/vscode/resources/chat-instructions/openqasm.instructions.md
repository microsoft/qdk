...
OpenQASM 3.0 programs can be imported into Python using the `qsharp.openqasm.import_openqasm` function and accessed via the `qsharp.code` module: `qsharp.openqasm.import_openqasm("qubit q; bit c; c = measure q;", name="Foo", program_type=qsharp.openqasm.ProgramType.File); qsharp.code.Foo()`.
